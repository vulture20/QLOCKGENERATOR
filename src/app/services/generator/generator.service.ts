...
export class GeneratorService {
  ...
  public async generatePreview(frontHeight: number, frontWidth: number, frontMirror: boolean, frontOutline: boolean,
    minutesPlacement: string, minutesRadius: number, minutesDistanceX: number, minutesDistanceY: number,
    alarmSymbol: string, alarmSize: number, alarmDistanceX: number, alarmDistanceY: number,
    matrixColumns: number, matrixRows: number, matrixDistanceX: number, matrixDistanceY: number,
    matrixLanguages: string, matrixText: string, matrixFont: string, matrixFontSize: number,
    logoText: string, logoTextSize: number, logoDistanceX: number, logoDistanceY: number,
    colorFront: string, colorLight: string) {

    this.generateFrontModel(frontHeight, frontWidth, frontMirror,
      minutesPlacement, minutesRadius, minutesDistanceX, minutesDistanceY,
      alarmSymbol, alarmSize, alarmDistanceX, alarmDistanceY,
      matrixColumns, matrixRows, matrixDistanceX, matrixDistanceY,
      matrixLanguages, matrixText, matrixFont, matrixFontSize,
      logoText, logoTextSize, logoDistanceX, logoDistanceY,
      colorFront, colorLight).then((front) => {
        // console.log("GeneratorService generatePreview() - generateFrontModel() - SUCCESS", front);
        let svg = makerjs.exporter.toSVG(front, this.getSvgRenderOptions(frontOutline, colorFront, colorLight));
  ...
  public async generateSVG(frontHeight: number, frontWidth: number, frontMirror: boolean, frontOutline: boolean,
    minutesPlacement: string, minutesRadius: number, minutesDistanceX: number, minutesDistanceY: number,
    alarmSymbol: string, alarmSize: number, alarmDistanceX: number, alarmDistanceY: number,
    matrixColumns: number, matrixRows: number, matrixDistanceX: number, matrixDistanceY: number,
    matrixLanguages: string, matrixText: string, matrixFont: string, matrixFontSize: number,
  ...
  public async generateDXF(frontHeight: number, frontWidth: number, frontMirror: boolean, frontOutline: boolean,
    minutesPlacement: string, minutesRadius: number, minutesDistanceX: number, minutesDistanceY: number,
    alarmSymbol: string, alarmSize: number, alarmDistanceX: number, alarmDistanceY: number,
    matrixColumns: number, matrixRows: number, matrixDistanceX: number, matrixDistanceY: number,
    matrixLanguages: string, matrixText: string, matrixFont: string, matrixFontSize: number,
  ...
  public async generatePNG(frontHeight: number, frontWidth: number, frontMirror: boolean, frontOutline: boolean,
    minutesPlacement: string, minutesRadius: number, minutesDistanceX: number, minutesDistanceY: number,
    alarmSymbol: string, alarmSize: number, alarmDistanceX: number, alarmDistanceY: number,
    matrixColumns: number, matrixRows: number, matrixDistanceX: number, matrixDistanceY: number,
    matrixLanguages: string, matrixText: string, matrixFont: string, matrixFontSize: number,
    ...
    let promise = new Promise((resolve, reject) => {
      this.generateFrontModel(frontHeight, frontWidth, frontMirror,
        ...
        colorFront, colorLight).then((front) => {
          ...
          svgToPng.svgAsPngUri(element.firstChild, {
            encoderOptions: 1,
            scale: 5.906
          }, (uri) => {
 ...
 */
  public async generatePDF(frontHeight: number, frontWidth: number, frontMirror: boolean, frontOutline: boolean,
    minutesPlacement: string, minutesRadius: number, minutesDistanceX: number, minutesDistanceY: number,
    alarmSymbol: string, alarmSize: number, alarmDistanceX: number, alarmDistanceY: number,
    matrixColumns: number, matrixRows: number, matrixDistanceX: number, matrixDistanceY: number,
    matrixLanguages: string, matrixText: string, matrixFont: string, matrixFontSize: number,
      ...
      this.generateFrontModel(frontHeight, frontWidth, frontMirror,
        ...
        colorFront, colorLight).then((front) => {
          ...
          let pdfOptions = {
            ...
            info: {
              Title: this.getFileName(frontHeight, frontWidth, "", matrixLanguages, false, matrixFont, alarmSymbol, logoText, frontOutline, frontMirror),
   ...
   */
  private calculateLettersCoordinate(height: number, width: number, columns: number, rows: number, startX: number, startY: number, ): ICoordinate[] {
    let grid: Array<{ "x": number, "y": number }> = [];
    let diffX = (width - (2 * startX)) / (columns - 1);
    let diffY = (height - (2 * startY)) / (rows - 1);
    ...
    for (let row = 0; row < rows; row++) {
      let y = height - ((row * diffY) + startY);
      for (let column = 0; column < columns; column++) {
        let x = (column * diffX) + startX;
        grid.push({ "x": x, "y": y });
      }
    }
  ...
  private calculateMinutesCoordinate(height: number, width: number, startX: number, startY: number, mode: string): ICoordinate[] {
    ...
    switch (mode) {
      case "horizontal":
        let diffX = (width - (2 * startX)) / 3;
        for (let minute = 0; minute < 4; minute++) {
          let x = (minute * diffX) + startX;
          let y = startY;
          grid.push({ "x": x, "y": y });
        }
        break;
      case "vertical":
        let diffY = (height - (2 * startY)) / 3;
        for (let minute = 0; minute < 4; minute++) {
          let x = startX;
          let y = (minute * diffY) + startY;
          grid.push({ "x": x, "y": y });
        }
        break;
      case "corner":
  ...
  private async generateFrontModelAddBackground(frontHeight: number, frontWidth: number) {
    ...
    let models: makerjs.IModel[] = [];

    let promise = new Promise((resolve, reject) => {
      // Add Rectangle as background
      let background: makerjs.IModel = new makerjs.models.Rectangle(frontWidth, frontHeight);
  ...
  private async generateFrontModelAddAlarm(frontHeight: number, frontWidth: number, alarmSymbol: string, alarmSize: number, alarmGrid: ICoordinate[]) {
    ...
    let promise = new Promise((resolve, reject) => {
      opentype.load(this.alarmFont, (error, font) => {
        if (error) {
          ...
          model.units = "Millimeter";
          let size = makerjs.measure.modelExtents(model);
          let width = size.center[0];
          let height = size.center[1];
      ...
      opentype.load(matrixFont, (error, font) => {
        if (error) {
          ...
          for (let i = 0; i < text.length && i < lettersGrid.length; i++) {
            ...
            if (size == undefined) {
              size = { "high": [0], "low": [0, 0], "center": [0, 0], "width": 0, "height": 0 }
            }
  ...
  private async generateFrontModel(frontHeight: number, frontWidth: number, frontMirror: boolean,
    minutesPlacement: string, minutesRadius: number, minutesDistanceX: number, minutesDistanceY: number,
    alarmSymbol: string, alarmSize: number, alarmDistanceX: number, alarmDistanceY: number,
    matrixColumns: number, matrixRows: number, matrixDistanceX: number, matrixDistanceY: number,
    matrixLanguages: string, matrixText: string, matrixFont: string, matrixFontSize: number,
    ...
    let resultBackground = await this.generateFrontModelAddBackground(frontHeight, frontWidth).then((models) => {
      front.models = front.models.concat(models);
    });

    // Add Minutes
    let minutesGrid = this.calculateMinutesCoordinate(frontHeight, frontWidth, minutesDistanceX, minutesDistanceY, minutesPlacement);
    let resultMinutes = await this.generateFrontModelAddMinutes(minutesGrid, minutesRadius).then((models) => {
      front.models = front.models.concat(models);
    });

    // Add Alarm
    if (alarmSymbol.length > 0) {
      let alarmGrid = this.calculateAlarmCoordinate(frontHeight, frontWidth, alarmDistanceX, alarmDistanceY);
      let resultAlarm = await this.generateFrontModelAddAlarm(frontHeight, frontWidth, alarmSymbol, alarmSize, alarmGrid).then((models) => {
        front.models = front.models.concat(models);
      });
    }

    // Add Text
    let text = this.prepareMatrixText(matrixText);
    let lettersGrid = this.calculateLettersCoordinate(frontHeight, frontWidth, matrixColumns, matrixRows, matrixDistanceX, matrixDistanceY);
    let resultText = await this.generateFrontModelAddText(matrixFont, matrixFontSize, text, lettersGrid).then((models) => {
      front.models = front.models.concat(models);
    });
   ...
   */
  getFileName(height: number, width: number, variant: string = null, language: string,
    isLanguageChanged: boolean = false, font: string, alarmValue: string, logoText: string,
    outline: boolean, mirror: boolean): string {
